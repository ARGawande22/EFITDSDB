...
Create   Procedure [dbo].[sel_DDOClient]
	@OfficeId int=0

--***********************************************************
--***
--*** Created On 27 Dec 2025 By A.R.Gawande
--***
--***********************************************************
AS
BEGIN
	...
	IF @OfficeId<=0
	BEGIN
		SELECT TOP 1 @OfficeId=Office_Id 
		FROM [dbo].[TDS_t_Office_Credentials] 
		WHERE [Status]='Y' AND Credential_Id=2 ORDER BY RAND()
	END
	...
	IF(@OfficeId=0 OR @OfficeId IS NULL)
	BEGIN
		Select @ErrMsg='No DDO valid login available'
		GOTO spError
	END
